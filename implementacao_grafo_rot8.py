...
class Grafo:
    ...
    def adiciona_aresta(self, a, peso):
        if peso < 1:
            raise PesoInvalidoException('O peso ' + peso + 'é invalido')
    ...
    def peso_aresta(self, x, y):
        indX = self.N.index(x)
        indY = self.N.index(y)
        return self.M[indX][indY] if self.M[indX][indY] > 0 else self.M[indY][indX]

    def vertices_adj(self, v):
        va = set()
        indxV = self.N.index(v)
        for i in range(len(self.N)):
            if (i != indxV):
                if (self.M[indxV][i] > 0):
                    va.add(self.N[i])
                elif (self.M[i][indxV] > 0):
                    va.add(self.N[i])
        return va
    ...
    def primModificado(self):
        ...
        # INICIANDO VARIAVEIS
        JET = dict.fromkeys(self.N, float('Inf'))

        PARENT = dict.fromkeys(self.N, None)

        Q = copy.deepcopy(self.N)

        menorAresta = float('Inf')

        menorArestaCoords = [None, None]

        # Buscando a aresta de menor peso
        for i in range(len(self.M)):
            for j in range(len(self.M)):
                if (self.M[i][j] < menorAresta and self.M[i][j] > 0):
                    menorAresta = self.M[i][j]
                    menorArestaCoords[0] = i
                    menorArestaCoords[1] = j
        # RAIZ
        X = self.N[menorArestaCoords[1]]
        ...
        while (len(Q) > 0):
            ...
            for vertice in Q:
                if (JET[vertice] < menorJET):
                    menorJET = JET[vertice]
                    vertMenorJET = vertice

            X = vertMenorJET
            Q.remove(X)

            for vertice in self.vertices_adj(X):
                if (vertice in Q and self.peso_aresta(vertice, X) < JET[vertice]):
                    JET[vertice] = self.peso_aresta(vertice, X)
                    PARENT[vertice] = X
